...
class NeuralNetwork:
    # Inner class alias for backward compatibility
    layer = Layer
    ...
    def __init__(self, inputs, outputs=None, labels=None, task="Regression", supress_neuralnetwork_warnings=False):
        # Set module level suppression in layers
        layers.supressWarnings = supress_neuralnetwork_warnings
        # Also need local suppression? The original code used a global supressWarnings.
        # I will store it in instance but it's used mainly in save/load which are instance methods or static.
        self.supressWarnings = supress_neuralnetwork_warnings
    ...
    def __activationVarToStr(self, l):
        var = l.activationVar
        if var == activations.sigmoid:
            if l.type == "INPUT":
                string = "None"
            else:
                string = "sigmoid"
        elif var == activations.tanh:
            string = "tanh"
        elif var == activations.LeakyReLU:
            string = "leakyReLU"
        elif var == activations.ReLU:
            string = "reLU"
        return string
        ...
        for l in data:
            if not ("Contents" in l.keys()):
                if "activation" in l.keys():
                    ...
                    myLayer = Layer(
                        nodes=l["nodes"], name=l["name"], activation=activations.sigmoid
                    )
                myLayer.type = l["type"]
                myLayer.key = l["key"]
        ...
        for epochCounter in myRange1:
            ...
            for batch_idx, batch in enumerate(batch_iter):
                ...
                if nn.task == "Regression":
                     ...
                     target_data = [d["target"] for d in batch]
                else:
                     input_data = [d["input"] for d in batch]
                ...
                targets.data = target_array_np.astype(np.float32)
                ...
                while i >= 1:
                    ...
                    # Hadamard product with Errors
                    gradients.simpleMultiply(Errors)

                    # Normalize gradients by batch size for stability
                    ...
                    bias_delta_matrix.data = bias_gradients
                    layer2.bias.add(bias_delta_matrix)
